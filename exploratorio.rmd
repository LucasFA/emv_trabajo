...
Esta es la descripción literal obtenida de Kaggle junto al conjunto de datos:
Personas:
...
De acuerdo a lo anterior, ya vamos a eliminar ciertas columnas del conjunto de datos

```{r}
data <- data[, -which(names(data) %in% c("ID", "Dt_Customer"))]
str(data)
```
...
<!-- print_factor_frequency <- function(column) { -->
<!--   if (is.factor(column)) { -->
<!--     cat("Frequency for", names(data)[which(data == column)], ":\n") -->
<!--     print(table(column)) -->
<!--     cat("\n") -->
<!--   } -->
<!-- } -->
...
normalizadas <- data.frame(scale(var_numericas))
# str(var_numericas)
boxplot(
  normalizadas,
  main = "Outliers",
  ...
  ylab = "Valor",
  col = c(seq_len(ncol(normalizadas))) # numero de columnas!
)
```
...
str(normalizadas)
colnames(normalizadas)
# for (colname in colnames(normalizadas)) {
#   qqnorm(
#     y = normalizadas$colname,
...
#   )
#   qqline(normalizadas$colname, distribution = qnorm)
# }
```
